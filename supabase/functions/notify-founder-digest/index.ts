...
//      window. If zero rows, marks the run skipped_zero (no email sent).
//   5. Composes the digest HTML — per-magnet counts, then the row table
//      with email + magnet + welcome variant + UTM context.
...
function buildDigestEmail(rows: CaptureRow[], digestDate: string): {
  subject: string;
  html: string;
  text: string;
} {
  ...
  const subject = `${total} new lead${total === 1 ? '' : 's'} — ${digestDate} · Eden Institute`;
...
Deno.serve(async (req) => {
  ...
  try {
    ...
    // ── Zero-capture path: mark skipped, no email ──
    if (!rows || rows.length === 0) {
      ...
      console.log(`notify-founder-digest: zero captures for ${digestDate} — no email sent`);
      return json(200, { sent: false, reason: 'zero_captures', digest_date: digestDate });
    }

    // ── Build + send digest ──
    const { subject, html, text } = buildDigestEmail(rows, digestDate);
    ...
    console.log(`notify-founder-digest: sent ${digestDate} digest`, JSON.stringify({ resendId: resendData?.id, count: rows.length }));
