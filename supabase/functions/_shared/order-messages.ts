...
function wrap(body: string): string {
  ...
  // emailWrapper, and correct its quiz-funnel footer reason for order messages.
  return emailWrapper(body)
    ...
    .join("You're receiving this because you placed an order at edeninstitute.health.");
}
...
export function buildPreorderConfirmationEmail(order: OrderRow): { subject: string; html: string } {
  ...
  const body =
    ...
    heading('Your order') +
    // The handle the customer needs for any later question, and the one /returns
    // instructs them to quote.
    (order.order_number
    ...
    // Two dates, deliberately. The first is what we aim for; the second is the binding
    // commitment the FTC delay-notice clock runs on. Both must stay visible: if only the
    // target were prominent, it could be treated as the stated shipping time.
...
function buildDeliveredEmail(order: OrderRow): { subject: string; html: string } {
  const item = order.product_label ? order.product_label : 'your order';
  const body =
    p(`Hi ${firstName(order)},`) +
    p(`Your <strong>${item}</strong> was delivered today.`) +
    p(`If it is not where you expected, check with anyone else at home first, then any `
      + `porch, side door, or mailroom the carrier might use. If it still does not turn `
      + `up, reply to this email and we will sort it out with you.`) +
    p(`When you are ready to begin, start with Week 1 and read it together. That is the `
      + `whole method: one plant, one week, at your own table.`) +
    p(`We prayed over this while we built it. It is a joy that it is finally with you.`) +
    signature();
  return { subject: "Your Eden's Table order has arrived", html: wrap(body) };
}
...
const REGISTRY: Partial<Record<OrderStatus, MessageDef[]>> = {
  ...
  delivered: [
    // Email FIRST, not SMS only. SMS consent is an unchecked box by default, so an
    // SMS-only delivered message means most buyers hear nothing when a $249 box lands
    // on their porch. This email is also the record if it goes missing.
    { channel: 'email', templateKey: 'delivered', from: 'shipped' },
