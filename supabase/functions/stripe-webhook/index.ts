...
import { recordPreorderFromSession, applyRefundByPaymentIntent, logPartialRefund, ResolvedLineItem } from "../_shared/order-flow.ts"
...
async function handleOneOffPayment(session: Stripe.Checkout.Session) {
  ...
  // order + one line item per cart line, transition to preorder_hold (or straight to
  // ready_to_fulfill once the product's is_preorder flips false), and fire the
