-- 20260916175000_pocket_materia_medica_hide_hidden_herbs.sql
--
-- 20260916170000 hid H238 Shankhpushpi (status 'Hidden - species identity under review 2026-09-15')
-- from herbs_directory_v, herbs_public and herbs_clinical_v. pocket_materia_medica() (practitioner
-- workspace, called by the practitioner-clinical edge function) reads public.herbs directly, so it
-- still offered H238. This adds the same filter. The body is the LIVE definition read on 2026-09-15
-- with exactly one added WHERE line; signature, SECURITY DEFINER, search_path and grants unchanged.

CREATE OR REPLACE FUNCTION public.pocket_materia_medica(p_person_profile_id uuid, p_encounter_id uuid, p_framework text DEFAULT NULL::text, p_pattern_id text DEFAULT NULL::text)
 RETURNS jsonb
 LANGUAGE plpgsql
 SECURITY DEFINER
...
BEGIN
  ...
  scored AS (
    SELECT h.herb_id,
           ...
           CASE WHEN bool_or(v.verdict = 'avoid') THEN 'avoid'
     ...
     WHERE coalesce(h.status, '') NOT LIKE 'Hidden%'  -- hidden herbs (20260916170000) never reach practitioners
     GROUP BY h.herb_id, h.common_name, h.latin_name, h.part_used,
              h.dosage_notes, h.preparation_methods, ct.items, ct.excluded
  )
  SELECT
...
do $assert$
declare
  def text := pg_get_functiondef('public.pocket_materia_medica(uuid, uuid, text, text)'::regprocedure);
begin
  if position('NOT LIKE ''Hidden%''' in def) = 0 then
    raise exception 'pocket_materia_medica() is missing the hidden-herb filter';
  end if;
  if has_function_privilege('anon', 'public.pocket_materia_medica(uuid, uuid, text, text)', 'EXECUTE') then
    raise exception 'anon must not execute pocket_materia_medica()';
  end if;
end
$assert$;
