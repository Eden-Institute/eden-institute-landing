-- Practitioner spec completion (founder request 2026-07-09, marketing-card
-- parity): the pocket materia medica now carries the herb's dosing guidance
-- and preparation methods so "recipe prescriptions, dosing guidance, and
-- blends" is answered on the card and on the printed formulary — not by a
-- second lookup. Dosage/preparation content is the existing Root-band herb
-- data (Lock #43-cited at the herb level); Practitioner ≥ Root in the tier
-- ladder, so no gate widens.
