...
// the ship-date constants below with the same file.

import { useEffect, useMemo, useState } from "react";
import { supabase } from "@/integrations/supabase/client";

// Two dates, and the split is load-bearing. SHIP_TARGET is what we aim for and talk
// about; SHIP_GUARANTEE is the BINDING commitment the FTC delay-notice clock runs on.
// Both must stay visible: if only the target were prominent it could be treated as the
// stated shipping time, pulling the deadline forward. Sync with _shared/order-config.ts.
  ...
  async function buy(sku: string) {
    ...
    try {
      ...
      if (fnError) {
        ...
        // The print run is finite. create-checkout refuses BEFORE creating a Stripe
        // session, so the buyer is never charged for a unit we cannot ship. Its message
        // names the product and remaining count, so surface it rather than a generic line.
            ...
            <div key={p.sku} className="rounded-lg border-2 bg-white flex flex-col p-6" style={{ borderColor: p.highlight ? "hsl(var(--eden-gold))" : "hsl(var(--border))" }}>
              ...
              <div className="mb-3">
                ...
                <p className="font-body text-xs mt-1" style={{ color: "hsl(var(--eden-sage))" }}>
                  {pre
                    ? (shipsOn ? `Preorder · ships on or before ${shipsOn}` : `Preorder · ships on or before ${SHIP_GUARANTEE}`)
              ...
              <button
                onClick={() => buy(p.sku)}
                disabled={busy !== null || notLive || soldOut}
        ...
        <div className="space-y-4">
          {anyPreorder ? (
            <>
              ...
              <TermCard title={`Aiming for ${SHIP_TARGET}. Guaranteed on or before ${SHIP_GUARANTEE}.`}>
                The first date is our honest target and it may move. The second is a promise: if
                your order has not shipped by {SHIP_GUARANTEE}, we will write to you before that
                date and you can cancel for a full refund. We will email you updates as production
                progresses.
              </TermCard>
              <TermCard title="Your card is charged today.">
                Your checkout total includes a flat $12 shipping charge and any sales tax. You may
                cancel for a full refund at any point before your order ships.
              </TermCard>
            </>
          ) : (
            <>
