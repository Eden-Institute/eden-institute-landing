 ...
 * gate (it returns NULL for columns above the caller's tier; see
 * supabase/migrations/20260916100000_herb_tier_model.sql), so this page
 * renders exactly the depth the caller is entitled to and advertises the
 * next tier. No herb is locked at any tier:
 *   • Free / anon → identity + energetics + tradition + every safety field;
 *     the clinical study renders as a preview with the Seed CTA.
 *   • Seed → + the clinical study; interactions, refer-out and sources
 *     render as a preview with the Root CTA.
 *   • Root / Practitioner → everything.
 * Which panel shows is decided by the tier from useHerbsDirectory.
 *
...
export default function HerbMonograph() {
  ...
  // Match badge, client-computed like HerbCard. temperature/moisture are
  // visible on every row, so free readers get the badge too (two-axis
  // degraded mode: tissue_states stays Seed-gated). Rows without axis data
  // classify Neutral and the badge hides without a special case.
  ...
  return (
    <div>
      ...
      >
        <div className="relative">
          {/* Heart sits absolute top-right of this relative container.
              No herb_id, no heart, same as HerbCard. */}
          {herb.herb_id && (
      ...
      <div className="max-w-3xl mx-auto px-6 py-10 space-y-10">
        ...
        {hasClinical && !hasRoot && (
          /* ── Seed caller: interactions, refer-out and sources are Root.
                The preview names exactly what Root adds to THIS herb;
                bullets come from TIER_DEPTH.root so they match the tier
                ladder on Start, Pricing and the tier cards. ── */
          <section
            className="p-6 md:p-8 border-2 rounded"
            style={{
            ...
            aria-label="Interactions, referral guidance, and sources, opens with Root"
          >
            <p
              className="font-accent text-xs tracking-[0.3em] uppercase mb-2"
              style={{ color: "hsl(var(--eden-gold-ink))" }}
            ...
            <h2
              ...
              style={{ color: "hsl(var(--eden-bark))" }}
            >
              Root opens the interactions, referral guidance, and sources for {name}.
            </h2>
            <ul className="font-body text-sm text-muted-foreground space-y-1.5 mb-6 list-disc pl-5">
              {TIER_DEPTH.root.map((item) => (
                <li key={item}>{item}</li>
              ))}
            </ul>
            <Button
              ...
              className="w-full sm:w-auto whitespace-normal h-auto py-3"
              asChild
              data-cta="monograph-clinical-root"
            >
              <Link to={`${ROUTES.APOTHECARY_PRICING}#tier-root`}>
