...
const WhyEden = () => {
  ...
  return (
    <main className="min-h-screen overflow-x-hidden">
      {/* NAV BAR */}
      <nav className="fixed top-0 left-0 right-0 z-50 bg-eden-parchment/95 backdrop-blur-sm border-b border-eden-gold/20">
        <div className="eden-container flex items-center justify-between py-3 px-6">
          ...
          <div className="flex items-center gap-6">
            <Link to="/" className="font-body text-sm text-muted-foreground hover:text-foreground transition-colors hidden sm:inline">
              Home
            </Link>
            <Link to="/why-eden" className="font-body text-sm text-eden-gold font-semibold hover:text-eden-gold/80 transition-colors hidden sm:inline">
              Why Eden
            </Link>
            <button
              onClick={() => setAssessmentModal(true)}
              className="font-body text-sm text-muted-foreground hover:text-foreground transition-colors hidden sm:inline"
            >
              Free Quiz
            </button>
          </div>
      ...
      {/* SECTION 1: HERO */}
      <section className="pt-28 pb-20 md:pt-36 md:pb-28 px-6 parchment-texture">
        <div className="eden-container text-center max-w-4xl mx-auto">
          <h1 className="font-serif text-3xl sm:text-4xl md:text-5xl lg:text-6xl font-bold text-foreground leading-tight mb-6 animate-fade-in">
            This Isn't Another Herbalism Course.
          </h1>
          <p className="font-body text-xl md:text-2xl text-muted-foreground italic mb-8 animate-fade-in" style={{ animationDelay: "0.15s", fontFamily: "'EB Garamond', 'Crimson Text', Georgia, serif" }}>
            It's the one that should have existed all along.
          </p>
          <div className="eden-divider" />
          <p className="font-body text-base md:text-lg text-muted-foreground max-w-3xl mx-auto mb-10 leading-relaxed animate-fade-in" style={{ animationDelay: "0.3s", fontFamily: "'EB Garamond', 'Crimson Text', Georgia, serif" }}>
          ...
          <Button variant="eden" size="xl" onClick={scrollToCourse} className="animate-fade-in" style={{ animationDelay: "0.45s" }}>
            → Explore the Program
          </Button>
        </div>
      </section>
      ...
      <section className="section-padding-lg" style={{ backgroundColor: "#2C3E2D" }}>
        <div className="eden-container max-w-5xl mx-auto">
          <h2 className="font-serif text-2xl sm:text-3xl md:text-4xl font-bold text-eden-parchment text-center leading-tight mb-12 md:mb-16">
          ...
          <div className="grid gap-8 md:gap-10">
            {[
              ...
              <div key={i} className="border-l-2 border-eden-gold pl-6 md:pl-8">
                <h3 className="font-serif text-xl md:text-2xl font-semibold text-eden-gold mb-3">
                  {problem.title}
                </h3>
                <p className="text-eden-parchment/80 text-base md:text-lg leading-relaxed" style={{ fontFamily: "'EB Garamond', 'Crimson Text', Georgia, serif" }}>
                  {problem.body}
                </p>
              </div>
            ))}
        ...
        <div className="eden-container max-w-5xl mx-auto">
          ...
          <div className="grid gap-6 md:gap-8">
            {differentiators.map((d) => (
              <div
                key={d.num}
                className="relative bg-card/60 border border-border rounded p-6 md:p-8 pl-8 md:pl-10 shadow-sm hover:shadow-md transition-shadow"
                style={{ borderLeftWidth: "4px", borderLeftColor: "#C5A44E" }}
              >
      ...
      <section id="course-overview" className="section-padding-lg" style={{ backgroundColor: "#5C7A5C" }}>
        <div className="eden-container max-w-5xl mx-auto">
          <div className="text-center mb-12 md:mb-16">
            <h2 className="font-serif text-2xl sm:text-3xl md:text-4xl font-bold text-eden-parchment leading-tight">
          ...
          <div className="grid md:grid-cols-2 gap-8 md:gap-10">
            {/* Tier 1 */}
            <div className="bg-eden-parchment rounded p-8 md:p-10 shadow-lg">
              <p className="font-accent text-xs tracking-[0.25em] uppercase text-eden-gold mb-3 font-semibold">
                Tier 1
              </p>
              ...
              <ul className="space-y-3 mb-8" style={{ fontFamily: "'EB Garamond', 'Crimson Text', Georgia, serif" }}>
                {[
                  ...
                  <li key={i} className="flex items-start gap-2 text-muted-foreground text-base">
                    <span className="text-eden-gold mt-1 shrink-0">✦</span>
                    {item}
                  </li>
                ))}
              </ul>
            ...
            <div className="bg-eden-parchment rounded p-8 md:p-10 shadow-lg">
              ...
              <p className="font-serif text-xl font-bold text-eden-gold">Coming Summer 2026</p>
            </div>
          </div>
          <p className="text-center text-eden-parchment/70 mt-10 text-base italic" style={{ fontFamily: "'EB Garamond', 'Crimson Text', Georgia, serif" }}>
              ...
              <ul className="space-y-4">
                {forYou.map((item, i) => (
                  <li key={i} className="flex items-start gap-3">
                    <Check className="w-5 h-5 text-eden-sage mt-0.5 shrink-0" />
      ...
      <section className="section-padding-lg" style={{ backgroundColor: "#2C3E2D" }}>
        <div className="eden-container max-w-3xl mx-auto text-center">
          <h2 className="font-serif text-2xl sm:text-3xl md:text-4xl font-bold text-eden-parchment leading-tight mb-8">
      ...
      <section className="section-padding-lg" style={{ backgroundColor: "#C5A44E" }}>
        <div className="eden-container max-w-4xl mx-auto text-center">
          ...
          <div className="flex flex-col sm:flex-row items-stretch sm:items-center justify-center gap-4 mb-10">
            <Button
              size="xl"
              className="font-serif font-semibold tracking-wide text-eden-gold border-2 hover:bg-eden-bark/90"
              style={{ backgroundColor: "#1E1E14", borderColor: "#1E1E14" }}
