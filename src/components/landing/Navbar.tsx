...
import { NAV_BUTTONS, NAV_LINKS, NAV_PODCAST, type SiteButton, type SiteLink } from "@/lib/navLinks";
...
//
// Layout, container, gaps and breakpoint are IDENTICAL to the Astro header
// (two rows from min-[1024px]: logo + buttons, then the links + the podcast
// button; max-w-[1340px]); a difference would shift the logo sideways, change
// the header height and flip the hamburger on and off mid-journey. See the
// Astro file for the width arithmetic.
//
...
export default function Navbar() {
  ...
  return (
    <header className="w-full bg-[#FAF8F3] border-b border-[#D6CDB8] sticky top-0 z-50">
      <div className="max-w-[1340px] mx-auto px-6 py-4 min-[1024px]:pb-2 flex items-center justify-between">
        ...
        <div className="hidden min-[1024px]:flex items-center gap-3">
          {NAV_BUTTONS.map((b) => (
            <NavAnchor
              key={b.href}
              link={b}
              className="text-sm font-sans px-5 py-2 rounded-sm tracking-wide transition-colors duration-200 min-h-[44px] inline-flex items-center whitespace-nowrap"
              style={buttonStyle(b)}
            />
          ))}
        </div>
        <button
          type="button"
          className="min-[1024px]:hidden text-[#3B4A3F] min-h-[44px] min-w-[44px] flex items-center justify-center"
      ...
      <nav className="hidden min-[1024px]:flex max-w-[1340px] mx-auto px-6">
        <div className="w-full flex items-center gap-6 border-t border-[#EAE3D2] pt-2 pb-3">
          {NAV_LINKS.map((link) => (
            <NavAnchor key={link.href} link={link} className={LINK_CLASS} />
          ))}
          <NavAnchor
            link={NAV_PODCAST}
            className="text-sm font-sans px-3 py-1 rounded-sm tracking-wide whitespace-nowrap transition-opacity duration-200 hover:opacity-80"
            style={buttonStyle(NAV_PODCAST)}
          />
        </div>
      </nav>
      {open && (
        <div className="min-[1024px]:hidden bg-[#FAF8F3] border-t border-[#D6CDB8] px-6 pb-6 pt-4 flex flex-col gap-5">
          {[...NAV_LINKS, NAV_PODCAST].map((link) => (
            <NavAnchor
              key={link.href}
              link={link}
              onClick={close}
